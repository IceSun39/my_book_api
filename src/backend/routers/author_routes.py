...
async def get_author(author_id: int, session: AsyncSession = Depends(get_session), current_user: User = Depends(get_current_user)):
    author = await author_service.get_author(session, author_id)
    return author

@author_router.post("/", response_model=AuthorResponse, status_code=201)
async def add_author(author: AuthorCreate, session: AsyncSession = Depends(get_session), admin_user: User = Depends(get_current_admin_user)):
    return await author_service.add_author(session, author)

@author_router.put("/{author_id}", response_model=AuthorResponse)
async def update_author(author_id: int, author_update: AuthorCreate, session: AsyncSession = Depends(get_session), admin_user: User = Depends(get_current_admin_user)):
    return await author_service.update_author(session, author_id, author_update)

@author_router.delete("/{author_id}", status_code=204)
async def delete_author(author_id: int, session: AsyncSession = Depends(get_session), admin_user: User = Depends(get_current_admin_user)):
